...
public class CouponInfoServiceImpl extends ServiceImpl<CouponInfoMapper, CouponInfo> implements CouponInfoService {
    ...
    public Boolean receive(Long customerId, Long couponId) {
        ...
        try {
            ...
            if(flag){
                ...
                //5、更新优惠券领取数量
                int row = couponInfoMapper.updateReceiveCountByLimit(couponId);
                if (row == 1) {
                    //6、保存领取记录
    ...
    public List<AvailableCouponVo> findAvailableCoupon(Long customerId, BigDecimal orderAmount){
        ...
        List<NoUseCouponVo> list1 = list.stream().filter(item -> item.getCouponType().intValue() == 1).collect(Collectors.toList());
        ...
        List<NoUseCouponVo> list2 = list.stream().filter(item -> item.getCouponType().intValue() == 2).collect(Collectors.toList());
